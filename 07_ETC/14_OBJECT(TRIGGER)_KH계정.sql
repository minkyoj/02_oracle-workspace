...
/*
    ...
    EX)
    회원탈퇴시 기존의 회원테이블에 데이터를 DELETE한 후 곧바로 탈퇴된 회원들만 따로 보관하는 테이블에 자동으로 INSERT 처리해야된다.
...
/*
    ...
    - 상품이 출고된 경우 => 해당상품 찾아서 재고수량 감소 UPDATE
    UPDATE TB_PRODUCT
...
-- 학생 테이블 생성
-- TB_STU
-- 컬럼 : 학번, 이름, 성별(M,F), 전화번호, 퇴졸여부(디폴트 재학)
...
CREATE TABLE TB_STU(
    STU_NO NUMBER,           -- 학번
    STU_NAME VARCHAR2(20),   -- 이름
    GENDER CHAR(1) CHECK(GENDER IN ('M','F')),         -- 성별
    PHONE VARCHAR2(20),      -- 전화번호
    STU_STATUS VARCHAR2(20) DEFAULT '재학'
);
...
-- 데이터 5개 만들기
INSERT INTO TB_STU
    VALUES(SEQ_STU_NO.NEXTVAL, '김김김', 'M', '010-1111-1111', DEFAULT);
INSERT INTO TB_STU
    VALUES(SEQ_STU_NO.NEXTVAL, '이이이', 'F', '010-2222-2222', '휴학');
INSERT INTO TB_STU
    VALUES(SEQ_STU_NO.NEXTVAL, '박박박', 'M', '010-3333-3333', DEFAULT);
INSERT INTO TB_STU
    VALUES(SEQ_STU_NO.NEXTVAL, '최최최', 'F', '010-4444-4444', '휴학');
INSERT INTO TB_STU
    VALUES(SEQ_STU_NO.NEXTVAL, '정정정', 'F', '010-5555-5555', DEFAULT);
...
CREATE TABLE TB_ABSENCE(
    ABS_NO NUMBER,           -- 휴학번호
    STU_NO NUMBER REFERENCES TB_STU,   -- 학번
    ABS_DATE DATE,         -- 휴학일자
    ABS_STATUS CHAR(1) DEFAULT 'Y' CHECK(ABS_STATUS IN ('Y','N'))      -- 휴학여부
);
