...
/*
    ...
    (무작정 다 조회를 해오는게 아니라 각 테이블간 연결고리로써의 데이터를 매칭해서 조회해야됨!!)
...
-- JOIN절에 같이 조회하고자 하는 테이블 기술 + 매칭시킬 칼럼에 대한 조건도 같이 기술
-- JOIN USING, JOIN ON
...
SELECT EMP_ID, EMP_NAME, JOB_CODE, JOB_NAME
FROM EMPLOYEE
JOIN JOB ON (JOB_CODE = JOB_CODE); -- 안됨
...
WHERE DEPT_CODE = DEPT_ID AND D.DEPT_TITLE = '인사관리부';
...
-- 2. 사번, 사원명, 부서명, 직급명, 지역명, 국가명, 해당 급여등급에서 받을 수 있는 최대금액 (다 조인)
...
FROM DEPARTMENT, EMPLOYEE E , JOB J , LOCATION L, NATIONAL N, SAL_GRADE S
WHERE DEPT_CODE = DEPT_ID AND LOCATION_ID = LOCAL_CODE
AND L.NATIONAL_CODE = N.NATIONAL_CODE AND J.JOB_CODE = E.JOB_CODE
AND E.SAL_LEVEL = S.SAL_LEVEL;

-->> ANSI
SELECT EMP_ID, EMP_NAME, DEPT_TITLE, JOB_NAME, LOCAL_NAME, NATIONAL_NAME, MAX_SAL
FROM DEPARTMENT
JOIN EMPLOYEE E ON (DEPT_CODE = DEPT_ID)
JOIN LOCATION L ON (LOCATION_ID = LOCAL_CODE)
JOIN NATIONAL N ON (L.NATIONAL_CODE = N.NATIONAL_CODE)
