...
SELECT * 
FROM EMP
WHERE SUBSTR(EMPNO, 4) IN (1,3,5,7,9);

--12. EMP테이블에서 사원명, 입사일 조회 (단, 입사일은 년도와 월을 분리 추출해서 출력)
SELECT ENAME, ENAME, TO_CHAR(HIREDATE, 'YYYY MM')AS"입사년도/월", TO_CHAR(HIREDATE, 'DD')AS"입사일"
FROM EMP;

--13. EMP테이블에서 9월에 입사한 직원의 정보 조회
SELECT *
FROM EMP
WHERE SUBSTR(TO_CHAR(HIREDATE, 'YYMMDD'), 3, 2) = '09';
