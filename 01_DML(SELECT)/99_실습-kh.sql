...
SELECT EMP_NAME, HIRE_DATE, PHONE
FROM EMPLOYEE
WHERE SAL_LEVEL = 'S1';
--8. EMPLOYEE테이블에서 실수령액(6번 참고)이
--5천만원 이상인 사원의 이름, 월급, 실수령액, 고용일 조회
SELECT EMP_NAME, SALARY, ((SALARY + SALARY * NVL(BONUS, 0))*12) - (((SALARY + SALARY * NVL(BONUS, 0))*12)*0.03) AS "실수령액",
        HIRE_DATE
FROM EMPLOYEE
WHERE ((SALARY + SALARY * NVL(BONUS, 0))*12) - (((SALARY + SALARY * NVL(BONUS, 0))*12)*0.03) >= 50000000;
--9. EMPLOYEE테이블에 월급이 4000000이상이고
...
SELECT *
FROM EMPLOYEE
WHERE HIRE_DATE <= '01/01/01' AND HIRE_DATE >= '90/01/01';
...
--13. EMPLOYEE테이블에서 전화번호 처음 3자리가 010이 아닌
--사원의 이름, 전화번호를 조회
SELECT EMP_NAME, PHONE
FROM EMPLOYEE
WHERE NOT PHONE LIKE'010%';

--14. EMPLOYEE테이블에서 메일주소 '_'의 앞이 4자이면서 DEPT_CODE가
--D9 또는 D6이고 고용일이 90/01/01 ~ 00/12/01이고,
--급여가 270만 이상인 사원의 전체를 조회
SELECT *
FROM EMPLOYEE
WHERE INSTR(EMAIL, '_') = '5' AND DEPT_CODE IN ('D9' , 'D6') AND
      HIRE_DATE <= '00/12/01' AND HIRE_DATE >= '90/01/01' AND
      SALARY >= 2700000;

--15. EMPLOYEE테이블에서 사원 명과 직원의 주민번호를 이용하여
--하이유의 생년, 생월, 생일 조회
SELECT SUBSTR(EMP_NO, 1, 2)AS "생년", SUBSTR(EMP_NO, 3, 2)AS "생월", SUBSTR(EMP_NO, 5, 2) AS "생일"
FROM EMPLOYEE
WHERE EMP_NAME = '하이유';
...
--23. EMPLOYEE테이블에서 사번이 201번인
--사원명, 주민번호 앞자리, 주민번호 뒷자리, 주민번호 앞자리와 뒷자리의 합 조회
SELECT EMP_NAME, SUBSTR(EMP_NO, 1, 6) AS "앞자리", SUBSTR(EMP_NO, 8, 7) AS "뒷자리", TO_NUMBER(SUBSTR(EMP_NO, 1, 6))+TO_NUMBER(SUBSTR(EMP_NO, 8, 7)) AS "앞자리 뒷자리 합"
FROM EMPLOYEE
WHERE EMP_ID = 201;
