...
/*
    * 서브쿼리의 구분
    ...
    - 다중행 서브쿼리 : 서브쿼리의 조회 결과값이 여러 행일때 (여러 행 한 열) => 동명이인 노옹철 2명일 때
    - 다중열 서브쿼리 : 서브쿼리의 조회 결과값이 한 행이지만 컬럼이 여러개일 때 (한 행 여러열)
    - 다중행 다중열 서브쿼리 : 서브쿼리의 조회 결과값이 여러행 여러열일 때 (여러행 여러열)
...
--1) 유재식 또는 윤은해 사원과 같은 직급의 사원들의 사번, 사원명, 직급코드, 급여
...
WHERE JOB_CODE = (SELECT JOB_CODE
                    FROM EMPLOYEE
                    WHERE EMP_NAME IN ('유재식', '윤은해')); -- = 이라고 쓰면 에러남 !! 여러행으로 조회 됐기 때문!
                    -- 만약에 결과값이 여러개 나올 것 같으면 그냥 안정빵으로 IN으로 가자
                    
-- 사원 => 대리 => 과장 => 차장 => 부장 ...
-- 2) 대리 직급임에도 불구하고 과장 직급 급여들 중 최소 급여보다 많이 받는 직원 조회
--    (사번, 이름, 직급, 급여)

-- 2_1) 먼저 과장직급인 사원들의 급여 조회
SELECT SALARY
...
-- 3) 과장 직급 임에도 불구하고 차장직급인 사원들의 모든 급여보다도 더 많이 받는 사원들의 사번, 사원명, 직급명, 급여
...
-- 단, 보너스 포함 연봉이 3000만원 이상인 사원들만 조회 ! 보너스포함 연봉이 절대 NULL이 나오지 않도록
...
SELECT EMP_NAME, DEPT_CODE, 연봉-- , MANAGER_ID : 이건 오류남 ORA-00904
FROM (SELECT EMP_ID, EMP_NAME, (SALARY + SALARY * NVL(BONUS,0))*12 AS "연봉", DEPT_CODE
FROM EMPLOYEE)
WHERE 연봉 >= 30000000;
...
FROM EMPLOYEE;
-- 공동 19위 그 뒤 순위는 20 => 마지막 순위랑 조회된 행수가 다를 수 있음
