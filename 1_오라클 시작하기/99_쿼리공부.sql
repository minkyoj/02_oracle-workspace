...
-- 문제점 : NULL값 비교할 때는 단순한 일반 비교연산자를 통해 비교할 수 없음!
-- 해결방법 : IS NULL / IS NOT NULL 연산자를 이용해서 비교해야됌!
...
-- 문제점 : 5개
-- (JOB_CODE = 'J7' OR JOB_CODE = 'J6') AND SALARY > 2000000 AND연산자가 먼저기때문에 안됌
-- EMAIL LIKE '____%' => 이거아님
-- AND BONUS IS NULL; => 낫널임
-- 주민번호 빠짐
-- 200 초과
-- 1. 연산자 우선순위에 따라서 AND 연산자가 먼저 수행됌.
-- 2. 보너스가 없는 데이터만 나오고 있음
-- 3. 여자가 아닌 데이터가 나오고 있다
-- 4. 와일드카드 _가 데이터의 값과 중복되어 결과가 이상하게 나오고 있다.
-- 5. 월급이 200만원이 안되는 애들 출력 + 200초과가 출력되도록 

-- 정답 쿼리
SELECT EMP_NAME, EMP_NO, JOB_CODE, DEPT_CODE, SALARY, BONUS
FROM EMPLOYEE
WHERE (JOB_CODE = 'J7' OR JOB_CODE = 'J6') AND SALARY >= 2000000
AND INSTR(EMAIL, '_')= '4' AND BONUS IS NOT NULL AND (SUBSTR(EMP_NO, 8, 1) = '2' OR SUBSTR(EMP_NO, 8, 1) = '4');

--EMAIL LIKE '___$_%' << INSTR(EMAIL,'_')='4' 랑같음