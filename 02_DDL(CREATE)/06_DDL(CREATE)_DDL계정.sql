...
/*
    * FOREIGN KEY(외래키) 제약조건
    ...
    > 컬럼레벨방식
    컬럼명 자료형 [CONSTAINT 제약조건명] REFERENCES 참조할 테이블명[(참조할 컬럼명)]
    
    > 테이블레벨방식
    [CONSTAINT 제약조건명] FOREIGN KEY(컬럼명) REFERENCES 참조할 테이블명[(참조할 컬럼명)]
...
CREATE TABLE MEM(
    ...
    PHONE VARCHAR2(10),
    EMAIL VARCHAR2(50),
    GRADE_ID NUMBER REFERENCES MEM_GRADE--(GRADE_CODE) -- 컬럼레벨방식 : 생략하면 프라이머리키로 자동으로참조
...
INSERT INTO MEM
VALUES(3, 'user03', 'pass03', '이강인', null, null, null, 40);
-- ORA-02291: integrity constraint (DDL.SYS_C007165) violated - parent key not found
...
WHERE GRADE_CODE = '10'; -- ***********************
-- ORA-02292: integrity constraint (DDL.SYS_C007165) violated - child record found
...
--> 부모테이블로부터 무조건 삭제가 안되게하는 "삭제제한" 옵션이 걸려있음!!
...
/*
    ...
    * 삭제옵션 : 부모테이블의 데이터 삭제시 그 데이터를 사용하고 있는 자식 테이블의 값을
    ...
    - ON DELETE SET NULL : 부모 데이터 삭제시 해당 데이터를 쓰고있는 자식데이터의 값을 NULL로 변경
...
-- alias : 별칭
-- 서브쿼리 SELECT절에 산술식 또는 함수식이 기술 된 경우 반드시 별칭 지정해야됨!!
...
/*
    * 테이블 다 생성된 후에 뒤늦게 제약조건 추가
    ...
    - NOT NULL    : ALTER TABLE 테이블명 ADD MODIFY 컬럼명 NOT NULL; **** 약간 특이
...
-- EMPLOYEE 테이블에 DEPT_CODE 외래키 제약조건 추가 ( 참조하는테이블(부모) : DEPARTMENT(DEPT_ID) )
