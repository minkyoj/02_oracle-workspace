...
SELECT JOB_CODE, COUNT(*) AS "총 사원수", COUNT(BONUS) AS "보너스를 받는 사원 수",
...
SELECT DEPT_CODE, COUNT(*) AS "총 사원수", COUNT(BONUS) AS "보너스를 받는 사원 수",
        SUM(SALARY) AS "급여 합", ROUND(AVG(SALARY)) AS "평균급여",
        MIN(SALARY) AS "최저급여", MAX(SALARY) AS "최대급여"
FROM EMPLOYEE
GROUP BY DEPT_CODE
...
-- 각 부서별 평균급여 조회 (부서코드, 평균급여)
SELECT DEPT_CODE AS "부서명", ROUND(AVG(SALARY)) "평균급여"
FROM EMPLOYEE 
GROUP BY DEPT_CODE
ORDER BY 1;
...
SELECT DEPT_CODE AS "부서명", ROUND(AVG(SALARY)) "평균급여"
FROM EMPLOYEE
WHERE AVG(SALARY) >= 3000000 -- 오류발생 !! (그룹함수로 조건제시시 WHERE절에서는 안됌!)
GROUP BY DEPT_CODE
ORDER BY 1;

SELECT DEPT_CODE AS "부서명", ROUND(AVG(SALARY)) "평균급여" --4
FROM EMPLOYEE -- 1
GROUP BY DEPT_CODE -- 2
HAVING AVG(SALARY) >= 3000000; -- 3;

-- 직급별 총 급여합 (단, 직급별 급여 합이 1000만원 이상인 직급만 조회)
-- => 직급코드, 급여합 조회
SELECT JOB_CODE AS "직급명", SUM(SALARY) AS "급여 합"
FROM EMPLOYEE
GROUP BY JOB_CODE
...
FROM EMPLOYEE
GROUP BY DEPT_CODE
HAVING COUNT(BONUS) = 0; -- 숫자기때문에 그냥 0으로 쓰자 '0'으로 썼음
...
-- 위의 쿼리문 대신 아래처럼 WHERE절에 OR 써도 해결 가능!!
SELECT EMP_ID, EMP_NAME, DEPT_CODE, SALARY
FROM EMPLOYEE
...
FROM EMPLOYEE
WHERE SALARY >= 3000000; -- 8개행 (선동일, 송종기, 노옹철, 유재식, 정중하, *심봉선, *대북혼, 전지연)
-- 각 쿼리문의 SELECT절에 작성되어있는 컬럼 개수 동일해야 됨!
...
FROM EMPLOYEE
WHERE SALARY >= 3000000; 
-- 칼럼 개수 뿐만 아니라 컬럼자리마다 동일한 타입으로 기술해야됨!!
