...
-- 문제점 : 5개
-- 1. OR 연산자와 AND연산자가 나열되어 있을 경우 AND 연산자 연산이 먼저 수행됨! 문제에서 요구한 내용으로 OR연산이 먼저 수행되어야 함!
-- 2. 급여값에 대한 비교가 잘못되어있음. >이 아닌 >=으로 비교해야됨
-- 3. 보너스가 있는 이라는 조건에 IS NULL이 아닌 IS NOT NULL로 비교해야됨!
-- 4. 여자에 대한 조건이 누락되어있음.
-- 5. 이메일에 대한 비교시 네번째 자리에 있는 _를 데이터값으로 취급하기 위해서는 새 와일드카드를 제시해야 되고 또한 ESCAPE로 등록까지 해야함!
...
-- 문제점 1. 사용자계정 생성은 무조건 관리자계정에서만 가능!!
-- 문제점 2. SQL문이 잘못되어있음! 비번까지 입력해야함!
...
-- 문제점. 사용자 계정 생성 후 최소한의 권한 부여가 안됐다.
