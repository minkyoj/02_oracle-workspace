...
-- 이건 실제있는 테이블이 아님!! 그래서 기상, 그래서 논리테이블인거임!
...
-- 뷰는 논리적인 가상테이블! 실질적인 데이터를 저장하고 있지 않음!
...
SELECT * FROM JOB; -- 베이스 테이블(실제 데이터가 담겨 있음)
...
-- INSERT (에러 ORA-00904: "JOB_NAME": invalid identifier)
INSERT INTO VW_JOB(JOB_CODE, JOB_NAME) VALUES ('J8', '인턴');

-- UPDATE (에러 ORA-00904: "JOB_NAME": invalid identifier)
UPDATE VW_JOB
SET JOB_NAME = '인턴'
WHERE JOB_CODE = 'J7';

-- DELETE (에러 ORA-00904: "JOB_NAME": invalid identifier)
DELETE FROM VW_JOB
WHERE JOB_NAME = '사원';
...
-- INSERT(에러)
INSERT INTO VW_JOB VALUES ('인턴'); -- 실제 베이스 테이블에 INSERT 시 (NULL, '인턴') 추가
...
-- DELETE (이 데이터를 쓰고 있는 자식 데이터가 존재하기 때문에 삭제 제한 / 단, 없다면 삭제 잘됨!!)
DELETE FROM VW_JOB
WHERE JOB_NAME = '사원';
-- ORA-02292: integrity constraint (KH.SYS_C007193) violated - child record found
...
-- 2) FORCE | "NOFORCE"
-- NOFORCE    : 서브쿼리에 기술된 테이블이 존재하는 테이블이어야만 뷰가 생성되게 하는(생략시 기본값)
CREATE OR REPLACE /*NOFORCE VIEW*/ VIEW VW_EMP
...
-- WITH CHECK OR REPLACE VIEW VW_EMP
